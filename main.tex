...
\emph{When no plus sign is shown between different keyboard keys, it means they should be pressed in sequence. Alternatively, a plus sign between them means they should be pressed at the same time.}

\emph{For modifier keys (and their combination such as \keys{ctrl \shift}) \keys{\shift}, \keys{ctrl}, \keys{alt} (Windows) or \keys{option} (Mac), and \keys{cmd} (Mac), the plus sign after them means to hold down the modifier key while pressing the next key. For example, \keys{ctrl + s} means to hold down the \keys{ctrl} key and press the \keys{s} key.}
